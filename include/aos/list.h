...
static __INLINE void __dlist_add(dlist_t *node, dlist_t *prev, dlist_t *next)
{
    node->next = next;
    node->prev = prev;
...
static __INLINE void dlist_add(dlist_t *node, dlist_t *queue)
{
    __dlist_add(node, queue, queue->next);
}

static __INLINE void dlist_add_tail(dlist_t *node, dlist_t *queue)
{
    __dlist_add(node, queue->prev, queue);
}

static __INLINE void dlist_del(dlist_t *node)
{
...
static __INLINE void dlist_init(dlist_t *node)
{
    node->next = node->prev = node;
}

static __INLINE void INIT_AOS_DLIST_HEAD(dlist_t *list)
{
    list->next = list;
    list->prev = list;
}

static __INLINE int dlist_empty(const dlist_t *head)
{
 ...
 * @param[in]  queue  the head for your list.
 */
static __inline int dlist_entry_number(dlist_t *queue)
{
	int num;
	dlist_t *cur = queue;  
...
static __inline int slist_empty(const slist_t *head)
{
    return !head->next;
}
